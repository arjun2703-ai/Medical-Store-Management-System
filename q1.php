...
$id = $_POST['id'];
$sql = "SELECT S.DRUG_NAME,S.DRUG_CODE,S.SHEETS,SUM(B.QUANTITY) AS SALES FROM STOCK_DETAILS S,BILLINGS B WHERE S.DRUG_CODE = B.DRUG_CODE AND B.DRUG_CODE = '$code' and S.ID = '$id'";
$result = $conn->query($sql);
if ($result->num_rows > 0) {
// output data of each row
while($row = $result->fetch_assoc()) {
echo "<tr><td>" . $row["DRUG_NAME"]. "</td><td>" . $row["DRUG_CODE"]    . "</td><br><td>"
. $row["SHEETS"]. "</td><td>" .$row["SALES"] . "</td></tr>";
}
echo "</table>";
} else { echo "0 results"; }
$conn->close();
